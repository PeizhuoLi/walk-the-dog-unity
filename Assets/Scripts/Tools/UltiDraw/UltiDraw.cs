...
public static class UltiDraw {
	...
	public class GUIRect {
		...
		public GUIRect SubRect(GUIRect rect) {
			return new GUIRect(X-W/2f+rect.X*W, Y-H/2f+rect.Y*H, rect.W*W, rect.H*H);
		}

		public GUIRect SubRectWithCorner(GUIRect rect)
		{
			return SubRect(new GUIRect(rect.X + rect.W/2, rect.Y + rect.H/2, rect.W, rect.H));
		}
	...
	public static void OnGUILabel(Vector2 center, Vector2 size, float scale, string text, Color color, Color background) {
		...
		style.font = FontType;
		style.alignment = TextAnchor.MiddleCenter;
		style.fontSize = Mathf.RoundToInt(ScreenSize.x * scale);
	...
	public static void OnGUILabel(Vector2 center, Vector2 size, float scale, string text, Color color) {
		if(ReturnGUI()) {return;};
		OnGUILabel(center, size, scale, text, color, Transparent);
	}
