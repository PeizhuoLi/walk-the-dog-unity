...
namespace AI4Animation {
	...
	public class Frame {
		...
		public float3 GetAngularVelocityRotVec(int index, bool mirrored, bool local=false)
		{
			if(Timestamp - Asset.GetDeltaTime() < 0f) {
				return MathExtensions.AngularVelocity(GetBoneTransformation(index, mirrored, local).GetRotation(), Asset.GetFrame(Timestamp + Asset.GetDeltaTime()).GetBoneTransformation(index, mirrored).GetRotation(), Asset.GetDeltaTime());
			} else {
				return MathExtensions.AngularVelocity(
					Asset.GetFrame(Timestamp - Asset.GetDeltaTime()).GetBoneTransformation(index, mirrored, local).GetRotation(), GetBoneTransformation(index, mirrored).GetRotation(), Asset.GetDeltaTime());
			}
		}

		public float3 GetAngularVelocityRotVec(string bone, bool mirrored, bool local=false)
		{
			return GetAngularVelocityRotVec(Asset.Source.FindBone(bone).Index, mirrored, local);
		}
		
		public (Frame, Frame) GetConsecutiveFrames() {
			if(Timestamp - Asset.GetDeltaTime() < 0f)
			{
				return (this, Asset.GetFrame(Timestamp + Asset.GetDeltaTime()));
			} else {
				return (Asset.GetFrame(Timestamp - Asset.GetDeltaTime()), this);
			}
		}
