...
namespace AI4Animation {
	...
	public class MotionEditor : MonoBehaviour {
		public enum SimplifyCharacterEnum
		{
			Off,
			Clown
		}
		...
		[NonSerialized] public HashSet<string> Visualize = new HashSet<string>();
		[NonSerialized] public HashSet<string> Inspect = new HashSet<string>();
		...
		public float TranslatX = 0f;

		public bool AutoPlay = false;
		public bool ShowCharacter = true;
		public SimplifyCharacterEnum SimpifyCharacter = SimplifyCharacterEnum.Off;
		public bool ShowInRootSpace = false;
		...
		public class AssetSession {
			...
			public Actor CreateActor() {
				...
				if (Editor.SimpifyCharacter is SimplifyCharacterEnum.Off)
					actor.Create(bones.ToArray());
				else if (Editor.SimpifyCharacter is SimplifyCharacterEnum.Clown)
				{
					var boneNames = new[]
					{
						"Hips", "LeftUpLeg", "LeftLeg", "LeftFoot", "LeftToeBase", "LeftFootMiddle1",
						"LeftFootMiddle1Site", "Spine", "Spine1", "Spine2", "Spine3", "LeftShoulder", "LeftArm", 
						"LeftForeArm", "LeftHand", "Neck", "Neck1", "Head", "HeadSite", "RightShoulder", "RightArm", 
						"RightForeArm", "RightHand", "RightUpLeg", "RightLeg", "RightFoot", "RightToeBase", 
						"RightFootMiddle1", "RightFootMiddle1Site"
					};
					actor.Create(boneNames);
				}
				return actor;
			}
		...
		public void LoadFrame(float timestamp) {
			if(GetSession() != null) {
				...
				GetSession().GetActor().GetRoot().position += new Vector3(TranslatX, 0, 0);
			}
		}
		...
		{
			return GetCurrentFrame().Index - 1;
		}
		...
		public class MotionEditor_Editor : Editor {
			...
			public override void OnInspectorGUI() {
				Target.TranslatX = EditorGUILayout.Slider("Translate X", Target.TranslatX, -2, 2);
						...
						using(new EditorGUILayout.VerticalScope ("Box")) {
							...
							if(Target.Settings) {
								...
								using(new EditorGUILayout.VerticalScope ("Box")) {
									...
									Target.PlayOnEnable = EditorGUILayout.Toggle("Play On Enable", Target.PlayOnEnable);
									...
									Target.SimpifyCharacter = (SimplifyCharacterEnum)EditorGUILayout.EnumPopup("Simplify Character", Target.SimpifyCharacter);
