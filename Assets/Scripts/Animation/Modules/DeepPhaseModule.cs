	...
	public class DeepPhaseModule : Module {
        ...
        public class CurveSeries {
            ...
            public CurveSeries(MotionAsset asset, Actor actor, TimeSeries timeSeries, 
                DeepPhasePipeline.CHANNEL_TYPE channelType, string tag=null) {
                Asset = asset;
                Actor = actor;
                TimeSeries = timeSeries;
                if (channelType is DeepPhasePipeline.CHANNEL_TYPE.Velocities or DeepPhasePipeline.CHANNEL_TYPE.Positions)
                {
                    NumChannels = 3;
                }
                else if (channelType is DeepPhasePipeline.CHANNEL_TYPE.Rotations)
                {
                    NumChannels = 9;
                }
                else if (channelType is DeepPhasePipeline.CHANNEL_TYPE.PhaseManifold)
                {
                    var phaseModule = asset.GetModule<DeepPhaseModule>(tag);
                    NumChannels = phaseModule.Channels.Length * 2;
                }
                else
                {
                    throw new NotSupportedException($"Given channel type {nameof(channelType)} is not supported.");
                }

                Compute(channelType, tag);
            }

            public void Compute(DeepPhasePipeline.CHANNEL_TYPE channelType, string tag=null) {
                RootModule rootModule = Asset.GetModule<RootModule>();
                int[] mapping = Asset.Source.GetBoneIndices(Actor.GetBoneNames());

                var numCurves = channelType == DeepPhasePipeline.CHANNEL_TYPE.PhaseManifold ? 1 : mapping.Length;
                Curves = new Curve[numCurves];
                for(int i=0; i<Curves.Length; i++) {
                    Curves[i] = new Curve(this, NumChannels);
                }
                ...
                void Compute(int i, bool mirrored)
                {
                    switch (channelType)
                    {
                        case DeepPhasePipeline.CHANNEL_TYPE.Positions:
                        {
                            float[] positions = mirrored ? Curves[i].MirroredValues : Curves[i].OriginalValues;
                            {
                                for(int j=0; j<Asset.Frames.Length; j++) {
                                    ...
                                    positions[3 * j] = relativePosition.x;
                                    positions[3 * j + 1] = relativePosition.y;
                                    positions[3 * j + 2] = relativePosition.z;
                                }
                            }
                            break;
                        }
                        case DeepPhasePipeline.CHANNEL_TYPE.Velocities:
                        {
                            float[] velocities = mirrored ? Curves[i].MirroredValues : Curves[i].OriginalValues;
                            {
                                for(int j=0; j<Asset.Frames.Length; j++) {
                                    ...
                                    velocities[3 * j] = velo.x;
                                    velocities[3 * j + 1] = velo.y;
                                    velocities[3 * j + 2] = velo.z;
                                }
                            }
                            // Low-Pass Filter
                            {
                                ...
                                {
                                    float[] v = Curves[i].GetSlice(c, mirrored);
                                    float[] x = Utility.Butterworth(v, Asset.GetDeltaTime(), TimeSeries.MaximumFrequency);
                                    Curves[i].SetSlice(x, c, mirrored);
                                }
                            }
                            break;
                        }
                        case DeepPhasePipeline.CHANNEL_TYPE.Rotations:
                        {
                            float[] rotations = mirrored ? Curves[i].MirroredValues : Curves[i].OriginalValues;
                            {
                                for (int j = 0; j < Asset.Frames.Length; j++) {
                                    Matrix4x4 spaceC = rootModule.GetRootTransformation(Asset.Frames[j].Timestamp, mirrored);
                                    Matrix4x4 jointSpace = Asset.Frames[j].GetBoneTransformation(mapping[i], mirrored);
                                    Matrix4x4 relativeSpace = jointSpace.TransformationTo(spaceC);
                                    rotations[9 * j] = relativeSpace.m00;
                                    rotations[9 * j + 1] = relativeSpace.m01;
                                    rotations[9 * j + 2] = relativeSpace.m02;
                                    rotations[9 * j + 3] = relativeSpace.m10;
                                    rotations[9 * j + 4] = relativeSpace.m11;
                                    rotations[9 * j + 5] = relativeSpace.m12;
                                    rotations[9 * j + 6] = relativeSpace.m20;
                                    rotations[9 * j + 7] = relativeSpace.m21;
                                    rotations[9 * j + 8] = relativeSpace.m22;
                                }
                            }
                            break;
                        }
                        case DeepPhasePipeline.CHANNEL_TYPE.PhaseManifold:
                        {
                            var phaseModule = Asset.GetModule<DeepPhaseModule>(tag);
                            if (phaseModule == null)
                            {
                                Debug.Log($"Phase module on motion {i} asset is null.");
                                break;
                            }
                            float[] manifold = mirrored ? Curves[i].MirroredValues : Curves[i].OriginalValues;
                            {
                                for (int j = 0; j < Asset.Frames.Length; j++)
                                {
                                    var phaseManifold = phaseModule.GetManifold(Asset.Frames[i].Timestamp, mirrored);
                                    for (int k = 0; k < phaseManifold.Length; k++)
                                    {
                                        manifold[NumChannels * j + 2*k] = phaseManifold[k].x;
                                        manifold[NumChannels * j + 2*k + 1] = phaseManifold[k].y;
                                    }
                                }
                            }
                            break;
            ...
            public float[] Collect(float timestamp, bool mirrored) {
                ...
                float[] values = new float[count * NumChannels];
                int idx = 0;
                for(int i=0; i<count; i++) {
                    int curve = i;
                    for (int j = 0; j < NumChannels; j++)
                    {
                        values[idx] = Curves[curve].GetValue(timestamp, mirrored)[j];
                        idx += 1;
                    }
                }
                return values;
            ...
            public float[] Sample(float timestamp, bool mirrored) {
                ...
                // int count = Curves.Length;
                // float[] values = new float[TimeSeries.Samples.Length * count];
                // int idx = 0;
                // for(int i=0; i<count; i++) {
                //     int curve = i;
            ...
            public class Curve {
                ...
                public Curve(CurveSeries series, int numChannels = 3) {
                    Series = series;
                    OriginalValues = new float[Series.Asset.Frames.Length * numChannels];
                    MirroredValues = new float[Series.Asset.Frames.Length * numChannels];
                    NumChannels = numChannels;
                }

                public float[] GetValues(bool mirrored) {
                    if(mirrored) {
                ...
                public float[] GetValue(float timestamp, bool mirrored) {
                    return GetValue(Series.Asset.GetFrame(timestamp).Index-1, mirrored);
                }

                public float[] GetValue(int index, bool mirrored)
                {
                    int start_point = index * NumChannels;
                    int end_point = start_point + NumChannels;
                    return GetValues(mirrored)[start_point..end_point];
                }

                public float[] GetSlice(int channelIdx, bool mirrored)
                {
                    var value = GetValues(mirrored);
                    float[] res = new float[Series.Asset.Frames.Length];
                    for (int i = 0; i < Series.Asset.Frames.Length; i++)
                    {
                        res[i] = value[i * NumChannels + channelIdx];
                    }
                    return res;
                }

                public void SetSlice(float[] to, int channelIdx, bool mirrored)
                {
                    var value = GetValues(mirrored);
                    for (int i = 0; i < Series.Asset.Frames.Length; i++)
                    {
                        value[i * NumChannels + channelIdx] = to[i];
                    }
                }
            }
        }
        ...
        public static void ComputeCurves(MotionAsset asset, Actor actor, TimeSeries timeSeries, 
            DeepPhasePipeline.CHANNEL_TYPE channelType, string tag=null) {
            Curves = new CurveSeries(asset, actor, timeSeries, channelType, tag);
        }
            ...
            if(Curves != null) {
                using(new EditorGUILayout.VerticalScope ("Box")) {
                    ...
                    for(int i=0; i<Curves.Curves.Length; i++) {
                        ...
                        //Values
                        float[] values = Curves.Curves[i].GetValues(editor.Mirror);
                        Color[] colors = { UltiDraw.Red, UltiDraw.Green, UltiDraw.Blue };

                        for (int j = 1; j < view.z; j++)
                        {
                            var prev_value = Curves.Curves[i].GetValue(view.x + j - 1 - 1, editor.Mirror);
                            var new_value = Curves.Curves[i].GetValue(view.x + j - 1, editor.Mirror);
                            for (int k = 0; k < Curves.NumChannels; k++)
                            {
                                prevPos.x = rect.xMin + (float)(j - 1) / (view.z - 1) * rect.width;
                                prevPos.y = rect.yMax -
                                            prev_value[k].Normalize(Curves.MinView, Curves.MaxView, 0f, 1f) *
                                            rect.height;
                                newPos.x = rect.xMin + (float)(j) / (view.z - 1) * rect.width;
                                newPos.y = rect.yMax - new_value[k].Normalize(Curves.MinView, Curves.MaxView, 0f, 1f) *
                                    rect.height;
                                UltiDraw.DrawLine(prevPos, newPos, colors[k]);
                            }
                        }
