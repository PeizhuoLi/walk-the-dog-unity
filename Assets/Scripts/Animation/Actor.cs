...
namespace AI4Animation {
	...
	public class Actor : MonoBehaviour {
		...
		public static Actor CreateActor(MotionAsset asset, Transform parentObj = null, string[] boneNmaes = null) {
			Transform root = new GameObject(asset.name).transform;
			if (parentObj != null) root.parent = parentObj;
		...
		public int NumValidBones => GetBoneNames().Length;
			...
			public override void OnInspectorGUI() {
				...
				using(new EditorGUILayout.VerticalScope ("Box")) {
					...
					if(Inspect) {
						...
						if (Utility.GUIButton("Print Skeleton Names as Array", UltiDraw.DarkGrey, UltiDraw.White))
						{
							var res = string.Empty;
							var boneNames = Target.GetBoneNames();
							for (int i = 0; i < boneNames.Length; i++)
							{
								res += '"' + boneNames[i] + "\", ";
							}
