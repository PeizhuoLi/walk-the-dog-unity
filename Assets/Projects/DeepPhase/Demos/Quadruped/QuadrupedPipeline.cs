...
namespace DeepPhase {
    public class QuadrupedPipeline : AssetPipelineSetup {
        ...
        public bool WriteCompleteDataset = false;
        ...
        public override void Inspector() {
            ...
            void ProcessMode()
            {
                WriteCompleteDataset = EditorGUILayout.Toggle("Write Complete Dataset", WriteCompleteDataset);
            }
        ...
        public override void Finish() {
            if(Mode == MODE.ProcessAssets) {
                ...
                if (WriteCompleteDataset)
                {
                    for (int i = 0; i < Pipeline.GetEditor().Assets.Count; i++)
                    {
                        var asset = MotionAsset.Retrieve(Pipeline.GetEditor().Assets[i]);
                        if (asset.name.Contains("ex")) continue;
                        asset.Export = true;
        ...
        // todo: regorganize this and change the quadruped and biped hierarchy.
        protected virtual void ProcessAssets(MotionAsset asset) {
